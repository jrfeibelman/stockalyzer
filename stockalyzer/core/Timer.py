...
class TimerManager:
    # TODO let's create decorator so that no functions are calling timer.start() -> should be timer manager that does that
    __slots__ = '_timers', '_callbacks'

    def __new__(cls):
        if not hasattr(cls, '_instance'):
            cls._instance = super().__new__(cls)
            cls._instance._timers = dict()
    ...
    # @classmethod
    # def create_timer_event(self, ctx : Context):
    #     return Event.create_timer_event(ctx)